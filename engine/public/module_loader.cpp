...
namespace anton_engine {
...
#endif
    if(!module.handle) {
      throw Exception(anton::String("Could not load module ") + module_name);
    }
    return module;
  }
  ...
  void* get_function_from_module(Module module, anton::String_View name)
  {
    ...
    void* func_ptr = reinterpret_cast<void*>(
      GetProcAddress(static_cast<HMODULE>(module.handle), name.data()));
#else
    void* func_ptr = dlsym(module.handle, name.data());
#endif

    if(!func_ptr) {
      throw Exception(anton::String("Could not load function ") + name);
    }
    return func_ptr;
  }
