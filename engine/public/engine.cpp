...
namespace anton_engine {
  ...
  static void window_resize_callback(windowing::Window* const, f32 const width,
                                     f32 const height, void*)
  {
  }

  static void mouse_button_callback(windowing::Window* const, Key const button,
                                    i32 const action, void*)
  {
  ...
  static void cursor_position_callback(windowing::Window* const window,
                                       f32 const param_x, f32 const param_y,
                                       void*)
  {
    Vec2 const dimensions = windowing::get_window_size(window);
  ...
  static void scroll_callback(windowing::Window* const, f32 const,
                              f32 const offset_y, void*)
  {
  ...
  static void joystick_config_callback(i32 const joy, i32 const joy_event,
                                       void*)
  {
    // if (joy_event == GLFW_CONNECTED && glfwJoystickIsGamepad(joy)) {
    //     anton::String joy_name(glfwGetJoystickName(joy));
    //     ANTON_LOG_INFO("Gamepad connected: " + joy_name + " " + anton::to_string(joy));
    // }
  }
  ...
  static void init()
  {
    ...
    Vec2 const window_dims = windowing::get_window_size(main_window);
    ...
    postprocess_info.color_buffers[1].internal_format =
      Framebuffer::Internal_Format::rgba8;
...
#include <core/serialization/serialization.hpp>
  ...
  static void load_world()
  {
    ...
    /* Handle<Shader> unlit_default_shader_handle = */ shader_manager->add(
      ANTON_MOV(unlit_default_shader));

    Mesh container = assets::load_mesh("barrel", 1);
    Material barrel_mat;
    {
      anton::Array<u8> pixels;
      // auto create_noise_texture = [](anton::Array<u8>& pixels) {
    ...
    Handle<Mesh> const box_handle = mesh_manager->add(ANTON_MOV(container));
    ...
    if constexpr(DESERIALIZE) {
      ...
      auto instantiate_box = [default_shader_handle, box_handle,
                              material_handle](Vec3 position,
                                               float rotation = 0) {
      ...
      Static_Mesh_Component& quad_sm =
        ecs->add_component<Static_Mesh_Component>(quad);
  ...
  static void render_frame(Framebuffer* const framebuffer,
                           Framebuffer* const postprocess_back,
                           Mat4 const view_mat, Mat4 const inv_view_mat,
                           Mat4 const projection_mat, Mat4 const inv_proj_mat,
                           ...
                           Vec2 const viewport_size)
  {
    ...
    rendering::render_scene(snapshot, camera_transform, view_mat,
                            projection_mat);
  ...
  static void loop()
  {
    ...
    for(Entity const entity: camera_mov_view) {
      auto [camera_mov, camera, transform] =
        camera_mov_view.get<Camera_Movement, Camera, Transform>(entity);
    ...
    for(Entity const entity: cameras) {
      ...
      if(camera.active) {
        Vec2 const window_dims = get_window_size(main_window);
        Mat4 const view_mat = get_camera_view_matrix(transform);
        Mat4 const inv_view_mat = math::inverse(view_mat);
        Mat4 const proj_mat =
          get_camera_projection_matrix(camera, window_dims.x, window_dims.y);
        Mat4 const inv_proj_mat = math::inverse(proj_mat);
        // TODO: Fix shitcode.
        render_frame(deferred_framebuffer, postprocess_back, view_mat,
                     inv_view_mat, proj_mat, inv_proj_mat, transform,
                     Vec2(window_dims.x, window_dims.y));
        anton::swap(postprocess_front, postprocess_back);
