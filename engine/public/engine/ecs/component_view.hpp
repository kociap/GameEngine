...
namespace anton_engine {
  ...
  public:
    ...
    class iterator {
      ...
      iterator(anton::Tuple<Component_Container<Components>*...> c,
               underlying_iterator_t b, underlying_iterator_t e)
        : _containers(c), _begin(b), _end(e)
      {
      }

    public:
      using value_type =
        typename anton::Iterator_Traits<underlying_iterator_t>::value_type;
      using reference =
        typename anton::Iterator_Traits<underlying_iterator_t>::reference;
      using pointer =
        typename anton::Iterator_Traits<underlying_iterator_t>::pointer;
      using difference_type =
        typename anton::Iterator_Traits<underlying_iterator_t>::difference_type;
      using iterator_category = anton::Forward_Iterator_Tag;
    ...
    private:
      ...
      {
        return (... && anton::get<Component_Container<Components>*>(_containers)
                         ->has(entity));
      }

    private:
      anton::Tuple<Component_Container<Components>*...> _containers;
  ...
  public:
    ...
    {
      i64 sizes[] = {
        anton::get<Component_Container<Components>*>(_containers)->size()...};
      i64 min = sizes[0];
      for(i64 i = 1; i < anton::size(sizes); ++i) {
    ...
    [[nodiscard]] decltype(auto) get(Entity const entity)
    {
      if constexpr(sizeof...(Ts) == 1) {
        return (...,
                anton::get<Component_Container<Ts>*>(_containers)->get(entity));
      } else {
        return anton::Tuple<Ts&...>(get<Ts>(entity)...);
      }
    }
    ...
    {
      static_assert(anton::is_invocable<Callable, Entity, Components&...> ||
                    anton::is_invocable<Callable, Components&...>);
      auto smallest_container = find_smallest_container();
      for(Entity const entity: *smallest_container) {
        if(has_all_components(entity)) {
          if constexpr(anton::is_invocable<Callable, Components&...>) {
  ...
  private:
    bool has_all_components(Entity entity)
    {
      return (
        ... &&
        anton::get<Component_Container<Components>*>(_containers)->has(entity));
    }

    // TODO add const support
    Component_Container_Base* find_smallest_container()
    {
      Component_Container_Base* conts[] = {
        static_cast<Component_Container_Base*>(
          anton::get<Component_Container<Components>*>(_containers))...};
      i64 min = conts[0]->size();
      i64 smallest_cont = 0;
      for(i64 i = 1; i < anton::size(conts); ++i) {
  ...
  public:
    ...
    void each(Callable&& callable)
    {
      static_assert(anton::is_invocable<Callable, Entity, Component&> ||
                    anton::is_invocable<Callable, Component&>);
      for(Entity entity: *static_cast<Component_Container_Base*>(_container)) {
        if constexpr(anton::is_invocable<Callable, Component&>) {
          callable(get(entity));
        } else {
