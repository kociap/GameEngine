...
namespace anton_engine {
  ...
  template<typename T>
  auto subtract_as_unsigned(T x, T y) -> anton::make_unsigned<T>
  {
    if constexpr(anton::is_unsigned<T>) {
      return x - y;
    } else {
      if(x >= 0 && y < 0) {
        return T(x) + T(-(y + 1)) + 1;
  ...
  {
    u64 const engine_range =
      subtract_as_unsigned<u64>(mersenne_engine.max(), mersenne_engine.min());
