...
namespace anton_engine::windowing {
  ...
  {
    mimas_fullscreen_window(window->mimas_window,
                            reinterpret_cast<Mimas_Display*>(display));
  }

  void _window_activate_callback(Mimas_Window* const, mimas_i32 const activated,
                                 void* const data)
  {
    Window* const window = reinterpret_cast<Window*>(data);
    window->active = activated;
    if(window->callbacks.window_activate) {
      window->callbacks.window_activate(window, activated,
                                        window->callbacks.window_activate_data);
    }
  }

  static anton::Fixed_Array<Key, 256> create_mimas_to_key_map()
  {
    anton::Fixed_Array<Key, 256> key_map(256, Key::none);
  ...
  static anton::Fixed_Array<i32, 256> create_key_to_mimas_map()
  {
    anton::Fixed_Array<i32, 256> key_map{256, MIMAS_KEY_UNKNOWN};
    key_map[static_cast<i32>(Key::left_mouse_button)] = MIMAS_MOUSE_LEFT_BUTTON;
    key_map[static_cast<i32>(Key::right_mouse_button)] =
      MIMAS_MOUSE_RIGHT_BUTTON;
    key_map[static_cast<i32>(Key::middle_mouse_button)] =
      MIMAS_MOUSE_MIDDLE_BUTTON;
    key_map[static_cast<i32>(Key::thumb_mouse_button_1)] =
      MIMAS_MOUSE_THUMB_BUTTON_1;
    key_map[static_cast<i32>(Key::thumb_mouse_button_2)] =
      MIMAS_MOUSE_THUMB_BUTTON_2;
    key_map[static_cast<i32>(Key::mouse_scroll)] = MIMAS_MOUSE_WHEEL;
    key_map[static_cast<i32>(Key::mouse_horiz_scroll)] =
  ...
  {
    static anton::Fixed_Array<i32, 256> key_map = create_key_to_mimas_map();
    Mimas_Key key = static_cast<Mimas_Key>(key_map[static_cast<u32>(k)]);
    return mimas_get_key(key);
  }

  void _window_key_callback(Mimas_Window* const, Mimas_Key const key,
                            Mimas_Key_Action const action, void* data)
  {
    static anton::Fixed_Array<Key, 256> key_map = create_mimas_to_key_map();
    Key const mapped_key = key_map[key];
    Window* const window = reinterpret_cast<Window*>(data);
    if(mapped_key != Key::none && window->callbacks.key) {
      window->callbacks.key(window, key_map[key], action,
                            window->callbacks.key_data);
    }
  }

  void _window_mouse_button_callback(Mimas_Window* const, Mimas_Key key,
                                     Mimas_Mouse_Button_Action action,
                                     void* data)
  {
    static anton::Fixed_Array<Key, 256> key_map = create_mimas_to_key_map();
    Key const mapped_key = key_map[key];
    ...
    if(mapped_key != Key::none && window->callbacks.mouse_button) {
      window->callbacks.mouse_button(window, key_map[key], action,
                                     window->callbacks.key_data);
    }
  }

  void _window_scroll_callback(Mimas_Window* const, mimas_i32 const d_x,
                               mimas_i32 const d_y, void* data)
  {
    Window* const window = reinterpret_cast<Window*>(data);
  ...
  void _cursor_callback(Mimas_Window* const, mimas_i32 const x,
                        mimas_i32 const y, void* data)
  {
    Window* const window = reinterpret_cast<Window*>(data);
    if(window->callbacks.cursor_pos) {
      window->callbacks.cursor_pos(window, x, y,
                                   window->callbacks.cursor_pos_data);
    }
  }
  ...
  Window* create_window(f32 const width, f32 const height, bool const decorated)
  {
    ...
    if(mimas_window) {
      ...
      mimas_set_window_activate_callback(mimas_window,
                                         _window_activate_callback, window);
      mimas_set_window_key_callback(mimas_window, _window_key_callback, window);
      mimas_set_window_cursor_pos_callback(mimas_window, _cursor_callback,
                                           window);
      mimas_set_window_mouse_button_callback(
        mimas_window, _window_mouse_button_callback, window);
      mimas_set_window_scroll_callback(mimas_window, _window_scroll_callback,
                                       window);
      return window;
    } else {
  ...
  void clip_cursor(Window* window, Rect<i32> const* rect)
  {
    mimas_clip_cursor(window->mimas_window,
                      reinterpret_cast<Mimas_Rect const*>(rect));
  }
  ...
  {
    mimas_set_cursor(window->mimas_window,
                     reinterpret_cast<Mimas_Cursor*>(cursor));
  }

  Cursor* create_standard_cursor(Standard_Cursor standard_cursor)
  {
    Mimas_Cursor* const cursor = mimas_create_standard_cursor(
      static_cast<Mimas_Standard_Cursor>(standard_cursor));
  ...
  void set_window_resize_callback(Window* const window,
                                  window_resize_function const cb,
  ...
  void set_window_activate_callback(Window* const window,
                                    window_activate_function const cb,
                                    void* const user_data)
  {
  ...
  void set_cursor_pos_callback(Window* const window,
                               cursor_pos_function const cb, void* user_data)
  {
    window->callbacks.cursor_pos = cb;
    window->callbacks.cursor_pos_data = user_data;
  }

  void set_mouse_button_callback(Window* const window,
                                 mouse_button_function const cb,
                                 void* user_data)
  {
  ...
  void set_scroll_callback(Window* const window, scroll_function const cb,
                           void* user_data)
  {
  ...
  void set_key_callback(Window* const window, key_function const cb,
                        void* user_data)
  {
  ...
  Vec2 get_window_pos(Window* const window)
  {
    i32 x;
    i32 y;
  ...
  void set_window_pos(Window* const window, Vec2 const pos)
  {
    mimas_set_window_content_pos(window->mimas_window, pos.x, pos.y);
  }

  void set_size(Window* const window, Vec2 const size)
  {
  ...
  OpenGL_Context* create_context(i32 const major, i32 const minor,
                                 OpenGL_Profile const profile)
  {
    Mimas_GL_Context* const ctx = mimas_create_gl_context(
      major, minor, static_cast<Mimas_GL_Profile>(profile));
  ...
  {
    mimas_make_context_current(window->mimas_window,
                               reinterpret_cast<Mimas_GL_Context*>(ctx));
  }
