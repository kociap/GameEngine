...
namespace anton_engine::rendering {
  ...
  class Face_Metrics {
  public:
    anton::String_View family_name;
    anton::String_View style_name;
  ...
  //
  Font_Face* load_face_from_file(anton::String_View path, u32 face_index);

  // Returns:
  // Strongly typed handle that uniquely identifies the face or nullptr if failed to load the face.
  //
  Font_Face* load_face(anton::Slice<u8 const> file_data, u32 face_index);
  ...
  //
  i64 compute_text_width(Font_Face* face, Font_Render_Info info,
                         anton::String_View text);

  anton::Array<Glyph> render_text(Font_Face* face, Font_Render_Info info,
                                  anton::String_View text);
} // namespace anton_engine::rendering
