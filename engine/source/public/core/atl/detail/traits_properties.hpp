...
namespace anton_engine::atl {
    ...
    //
    template <typename T>
    struct Is_Bounded_Array: atl::False_Type {};

    template <typename T, usize N>
    struct Is_Bounded_Array<T[N]>: atl::True_Type {};
    ...
    // Is_Unbounded_Array
    template <typename T>
    struct Is_Unbounded_Array: atl::False_Type {};

    template <typename T>
    struct Is_Unbounded_Array<T[]>: atl::True_Type {};
    ...
    //
    template <typename T>
    struct Rank: atl::Integral_Constant<usize, 0> {};

    template <typename T>
    struct Rank<T[]>: atl::Integral_Constant<usize, Rank<T>::value + 1> {};

    template <typename T, usize N>
    struct Rank<T[N]>: atl::Integral_Constant<usize, Rank<T>::value + 1> {};
    ...
    struct Extent: atl::Integral_Constant<usize, 0> {};

    template <typename T>
    struct Extent<T[], 0>: atl::Integral_Constant<usize, 0> {};

    template <typename T, usize N>
    struct Extent<T[], N>: atl::Integral_Constant<usize, Extent<T, N - 1>::value> {};

    template <typename T, usize E>
    struct Extent<T[E], 0>: atl::Integral_Constant<usize, E> {};

    template <typename T, usize E, usize N>
    struct Extent<T[E], N>: atl::Integral_Constant<usize, Extent<T, N - 1>::value> {};
    ...
    //
    template <typename T>
    struct Is_Reference: atl::False_Type {};

    template <typename T>
    struct Is_Reference<T&>: atl::True_Type {};
    ...
    struct Is_Any_Of: atl::conditional<(... || atl::is_same<T, Types>), atl::True_Type, atl::False_Type> {};
    ...
    namespace detail {
        ...
        class Is_Convertible_Helper<From, To, false> {
            ...
            template <typename F, typename T, typename = decltype(test_aux<T>(atl::declval<F>()))>
            static True_Type test(int);
    ...
    namespace detail {
        ...
        // If T is a reference or atl::declval<T&>().~T() is well formed return true.
        // otherwise, return false.
        //
        template <typename T, bool = disjunction<Is_Void<T>, Is_Unbounded_Array<T>, Is_Function<T>>>
        struct Is_Destructible {
            using type = atl::False_Type;
        };

        template <typename T>
        struct Is_Destructible<T, false> {
        private:
            template <typename U, typename = atl::void_trait<decltype(atl::declval<U&>().~U())>>
            static atl::True_Type test(int);
...
#endif // !CORE_ATL_DETAIL_TRAITS_PROPERTIES_HPP_INCLUDE
