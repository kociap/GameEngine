...
namespace anton_engine::imgui {
    ...
    atl::Slice<Draw_Command const> get_viewport_draw_commands(Context& ctx, Viewport&);

    atl::Slice<Vertex const> get_vertex_data(Context& ctx);
    atl::Slice<u32 const> get_index_data(Context& ctx);

    void begin_window(Context& ctx, atl::String_View);
    ...
    void begin_widget(Context& ctx, atl::String_View identifier, Widget_Style options);
    ...
    Button_State button(Context& ctx, atl::String_View text, Button_Style style, Button_Style hovered_style, Button_Style active_style);
    void image(Context& ctx, u64 texture, Vector2 size, Vector2 uv_top_left, Vector2 uv_bottom_right);
    ...
    //
    void set_window_size(Context& ctx, Vector2 size);
    ...
    // Otherwise this function has no effect.
    //
    void set_window_pos(Context& ctx, Vector2 screen_pos);
