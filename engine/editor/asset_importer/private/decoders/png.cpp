...
static uint32_t read_uint32(uint8_t const* const stream, uint64_t& pos) {
...
static Chunk_Data read_chunk(uint8_t const* const stream, uint64_t& pos) {
    uint32_t const data_length = read_uint32(stream, pos);
    uint32_t const chunk_type = read_uint32(stream, pos);
    uint8_t const* const data = read_bytes(stream, data_length, pos);
    uint32_t const crc = read_uint32(stream, pos);
...
static Image_Header reinterpret_bytes_as_image_header(uint8_t const* bytes) {
    ...
    header.width = read_uint32(bytes, stream_pos);
    header.height = read_uint32(bytes, stream_pos);
...
Image_Data decode_png(containers::Vector<uint8_t> const& png_data) {
    ...
    containers::Vector<Indexed_Color> palette_colors(1 << 8);
    for (bool end_loop = false, iCCP_present = false, sRGB_present = false, PLTE_present = false, IDAT_read = false; !end_loop;) {
        ...
        switch (chunk_data.chunk_type) {
            ...
            case chunk_PLTE:
                ...
                memory::copy(chunk_data.data, chunk_data.data + chunk_data.data_length, reinterpret_cast<uint8_t*>(palette_colors.data()));
                palette_colors.resize(palette_entries);
                PLTE_present = true;
                break;
            case chunk_IDAT: {
                std::cout << "IDAT chunk\nLength: " << chunk_data.data_length << '\n';
    ...
    if (header.color_type == color_type_indexed) {
        uint64_t indexed_pixel_width = 3; // 3 because rgb with no alpha support yet
        ...
        for (uint8_t pixel_index: pixels_unfiltered) {
            auto color = palette_colors[pixel_index];
    ...
    return {header.width, header.height, pixel_format, std::move(pixels_unfiltered)};
}