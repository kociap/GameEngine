...
namespace anton_engine::asset_importing {
  ...
  [[nodiscard]] static Matching_Format
  get_matching_texture_format(importers::Image const& image)
  {
    ...
    case importers::Image_Pixel_Format::grey16: {
      Internal_Format const internal_format =
        image.pixel_format == importers::Image_Pixel_Format::grey16
          ? Internal_Format::r16
          : Internal_Format::r8;
      return {
        opengl::Format::red, internal_format, {GL_RED, GL_RED, GL_RED, GL_ONE}};
    }
    case importers::Image_Pixel_Format::grey8_alpha8:
    case importers::Image_Pixel_Format::grey16_alpha16: {
      Internal_Format const internal_format =
        image.pixel_format == importers::Image_Pixel_Format::grey16_alpha16
  ...
  [[nodiscard]] static i32 compute_number_of_mipmaps(u32 const width,
                                                     u32 const height)
  {
    i32 mipmap_levels = 0;
  ...
  static void write_texture(anton::String_View const output_directory,
                            anton::String_View const file_original_path,
                            importers::Image const& image)
  {
    ...
    format.sized_internal_format =
      utils::enum_to_value(texture_format.internal_format);
    format.mip_levels = compute_number_of_mipmaps(format.width, format.height);
    memcpy(format.swizzle_mask, texture_format.swizzle_mask,
           sizeof(texture_format.swizzle_mask));
    ...
    i64 const texture_chunk_size =
      static_cast<i64>(sizeof(Texture_Format)) + 8 + image_bytes;

    anton::String_View const out_filename_no_ext =
      anton::fs::get_filename_no_extension(file_original_path);
    anton::String const out_filename =
      anton::String(out_filename_no_ext) + ".getex";
    anton::String const out_file_path =
      anton::fs::concat_paths(output_directory, out_filename);
    FILE* file = fopen(out_file_path.data(), "wb");
    if(!file) {
      throw Exception(u8"Could not open file for writing");
    }
    fwrite(reinterpret_cast<char const*>(&texture_chunk_size), 8, 1,
           file); // Allows to skip the entire texture chunk without parsing it
    fwrite(reinterpret_cast<char const*>(&identifier), 8, 1, file);
    fwrite(reinterpret_cast<char const*>(&format), sizeof(Texture_Format), 1,
           file);
    fwrite(
      reinterpret_cast<char const*>(&image_bytes), 8, 1,
      file); // Prefix the image with its size in bytes because that will simplify mipmap access
    fwrite(reinterpret_cast<char const*>(image.data.data()), image_bytes, 1,
           file);
    fclose(file);
  }

  void import_image(anton::String_View const path)
  {
    ...
    anton::Array<u8> const file = utils::read_file_binary(path);
  ...
  {
    Vec3 const delta_pos1 = vert2.position - vert1.position;
    Vec3 const delta_pos2 = vert3.position - vert1.position;
    Vec2 const delta_uv1 = vert2.uv_coordinates - vert1.uv_coordinates;
    Vec2 const delta_uv2 = vert3.uv_coordinates - vert1.uv_coordinates;
    float const determinant =
      delta_uv1.x * delta_uv2.y - delta_uv1.y * delta_uv2.x;
    Vec3 const tangent = math::normalize(
      (delta_uv2.y * delta_pos1 - delta_uv1.y * delta_pos2) / determinant);
    Vec3 const bitangent = math::normalize(
      (delta_uv1.x * delta_pos2 - delta_uv2.x * delta_pos1) / determinant);

    vert1.tangent = vert2.tangent = vert3.tangent = tangent;
    vert1.bitangent = vert2.bitangent = vert3.bitangent = bitangent;
    //vert1.bitangent = vert2.bitangent = vert3.bitangent = Vec3::cross(vert1.normal, vert1.tangent);
  }

  static Mesh process_mesh(importers::Mesh const& imported_mesh)
  {
    anton::Array<Vertex> vertices(imported_mesh.vertices.size());
    ...
    if(imported_mesh.texture_coordinates.size() != 0) {
      for(isize i = 0; i < imported_mesh.vertices.size(); ++i) {
        ...
        vertices[i].uv_coordinates = Vec2(imported_mesh.texture_coordinates[i]);
      }
    } else {
      for(isize i = 0; i < imported_mesh.vertices.size(); ++i) {
        vertices[i].position = imported_mesh.vertices[i];
    ...
    for(isize i = 0; i < indices.size(); i += 3) {
      compute_tangents(vertices[indices[i]], vertices[indices[i + 1]],
  ...
  Imported_Meshes import_mesh(anton::String_View const path)
  {
    anton::Array<u8> const file = utils::read_file_binary(path);
    anton::String_View const extension = anton::fs::get_extension(path);
    if(importers::test_obj(extension, file)) {
      anton::Array<importers::Mesh> meshes = importers::import_obj(file);
  ...
  void save_meshes(anton::String_View const filename,
                   anton::Slice<u64 const> const guids,
                   anton::Slice<Mesh const> meshes)
  {
    anton::String const file_name_with_ext =
      anton::String(filename) + u8".mesh";
    anton::String const path =
      anton::fs::concat_paths(paths::assets_directory(), file_name_with_ext);
    ...
    for(isize i = 0; i < guids.size(); ++i) {
      ...
      fwrite(reinterpret_cast<char const*>(meshes[i].vertices.data()),
             vertex_count * sizeof(Vertex), 1, out);
      i64 const index_count = meshes[i].indices.size();
      fwrite(reinterpret_cast<char const*>(&index_count), sizeof(i64), 1, out);
      fwrite(reinterpret_cast<char const*>(meshes[i].indices.data()),
             index_count * sizeof(u32), 1, out);
    }
    fclose(out);
  }
} // namespace anton_engine::asset_importing
