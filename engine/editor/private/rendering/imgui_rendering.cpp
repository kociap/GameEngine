...
namespace anton_engine::imgui {
  ...
  static u32 vao;
  static anton::Array<Draw_Elements_Command> draw_commands;
  ...
  void setup_rendering()
  {
    ...
    glBufferStorage(GL_ARRAY_BUFFER, gpu_vertex_buffer.size, 0,
    ...
    glBufferStorage(GL_ELEMENT_ARRAY_BUFFER, gpu_element_buffer.size, 0,
                    GL_DYNAMIC_STORAGE_BIT | buffer_flags);
    ...
    glBufferStorage(GL_DRAW_INDIRECT_BUFFER, gpu_draw_cmd_buffer.size, 0,
                    GL_DYNAMIC_STORAGE_BIT | buffer_flags);
  ...
  void commit_draw()
  {
    if(draw_commands.size() > 0) {
      if(draw_cmd_buffer.head - draw_cmd_buffer.buffer + draw_commands.size() >
         draw_cmd_buffer.size) {
        draw_cmd_buffer.head = draw_cmd_buffer.buffer;
      }

      memcpy(draw_cmd_buffer.head, draw_commands.data(),
             draw_commands.size() * sizeof(Draw_Elements_Command));
